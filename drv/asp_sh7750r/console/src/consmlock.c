...
/*
 *	consmlock.c	Console/Low-level serial I/O driver [HEW]
 *
...
#pragma	inline(BTS)
static	BOOL	BTS( UINT *val, INT no )
{
	UINT	imask;
	UINT	b;
	UINT	bm = 1 << no;
...
#pragma	inline(BR)
static	void	BR( UINT *val, INT no )
{
	UINT	imask;
...
EXPORT	ER	consCreateMLock( FastMLock *lock, UB *name )
{
	...
	if ( name == NULL ) {
		...
		strncpy((B*)&cflg.exinf, (B*)name, sizeof(cflg.exinf));
	}
