...
LOCAL	ER	fatChainCluster(FATFS *fs, UW clno, CLAD *clad, VW info)
{
	...
	if (err >= E_OK) {
		...
		fs->ff_nextfree = clad->ca_no + clad->ca_len - 1;
	}
exit1:
	(void)fatMapEnd(map);
...
LOCAL	ER	fatGetRootINODE(FATFS *fs, FATNODE ** inodep)
{
	...
	if (inode->fino_ftype != FAT_SUBDIR) {
		...
		inode->fino_c.ino_path = (B*)inode_root_path;
...
LOCAL	void	fatGetDateTime(FAT_DIRENT *dirent, struct stat64_us *sb)
{
	...
	mtim.tm_sec = (dirent->de_mtime & FAT_TIME_MASK_SEC) << 1;
	mtim.tm_min = (dirent->de_mtime & FAT_TIME_MASK_MIN) >>
						FAT_TIME_SHIFT_MIN;
	mtim.tm_hour = (dirent->de_mtime & FAT_TIME_MASK_HOUR) >>
						FAT_TIME_SHIFT_HOUR;
	mtim.tm_mday = dirent->de_mdate & FAT_TIME_MASK_SEC;
	mtim.tm_mon = ((dirent->de_mdate & FAT_DATE_MASK_MON) >>
						FAT_DATE_SHIFT_MON) - 1;
	mtim.tm_year = ((dirent->de_mdate & FAT_DATE_MAS_YEAR) >>
				FAT_DATE_SHIFT_YEAR) + FATYEAR_TO_TMYEAR;

	memset(&atim, 0, sizeof(struct tm));
	atim.tm_mday = dirent->de_adate & FAT_TIME_MASK_SEC;
	atim.tm_mon = ((dirent->de_adate & FAT_DATE_MASK_MON) >>
				FAT_DATE_SHIFT_MON) -1;
	atim.tm_year = ((dirent->de_adate & FAT_DATE_MAS_YEAR) >>
				FAT_DATE_SHIFT_YEAR) + FATYEAR_TO_TMYEAR;
...
LOCAL	ER	fatUpdateFileSystem(FATFS *fs)
{
	...
	if (fs->ff_fsinfo > 0) {
		...
		/* Execute no processing when there are no changes */
		if (fs->ff_nextfree != CLVOID) {
...
LOCAL	ER	fatCheckFileSystem(FATFS *fs)
{
	...
	if (fsinfo > 0) {
		...
		/* Check signature */
		if (	fsi->fsi_leadsig == FSINFO_LEADSIGN_VAL &&
			fsi->fsi_structsig == FSINFO_STRUCTSIGN_VAL &&
			fsi->fsi_trailsig == FSINFO_TAILSIGN_VAL) {
			fs->ff_fsinfo = fsinfo;		/* Active */

			/* Note: Do not use the free cluster count
				(fsi->fsi_freecl), but update it. */
		}
		/* Initialize the start position to search free cluster
			(no change shall be set) */
		fs->ff_nextfree = CLVOID;
	}
	err = E_OK;
exit1:
	fimp_free(blkbuf);
...
LOCAL	ER	fat_lookup(FATFS *fs, LPINF *lp, const B *r_path)
{
	...
	len = strlen(curdir->cd_inode->ino_path);
	...
	if (fat_strncasecmp(curdir->cd_inode->ino_path, path, len) == 0
		...
		if (path[len] == '\0') {
			/* Current directory */
			if ((cpath = strrchr(path, '/')) != NULL) cpath++;
			else	cpath = path;
	...
	dtp = FAT_SUBDIR;
	dir.di_dirent = NULL;
	for ( ; (npath = strchr(cpath, '/')) != NULL; cpath = npath + 1) {
	...
	/* Search file */
	inode = NULL;
	err = fatSearchDIR_nm(fs, &dir, cpath, strlen(cpath), AC_RDONLY, NULL);
	if (err >= E_OK || err == EX_NOENT) {
		err = fatGetINODE(fs, &dir, dtp, path, strlen(path), &inode);
	}
				...
				struct dirent *ent, W *alen, W bflen)
{
	...
	if (ino <= CLSTART) {	/* Special for root "." and ".." */
		...
		nmlen = fatEncUtf16strToUtf8str((UH*)nm,
				fat_strlen16((UH*)nm), e->d_name, nmsz + 1);
	}
...
LOCAL	ER	fat_fchdir(FD *fd, UB *utf8nm, W utf8len)
{
	...
	*utf8nm++ = '/';
	(void)strcpy(utf8nm, fs->ff_c.fs_coninf->connm);
	if (path[1] == '\0') goto exit0;	/* root directory */

	utf8nm += strlen(utf8nm);
...
static const	UW	functab[] = {
		(UW)fatfs_open		+ 0x01,	/* +0x01: Breakable	*/
		(UW)fatfs_close		+ 0x01,
		(UW)fatfs_ioctl,
		(UW)fatfs_getdents,
		(UW)fatfs_chdir		+ 0x01,
		(UW)fatfs_fchdir	+ 0x01,
		(UW)fatfs_fsync		+ 0x01,
		(UW)fatfs_unlink,
		(UW)fatfs_rename,
		(UW)fatfs_chmod,
		...
		(UW)fatfs_mkdir,
		(UW)fatfs_rmdir,
		(UW)fatfs_statvfs	+ 0x01,
		(UW)fatfs_fstatvfs	+ 0x01,
		(UW)fatfs_sync		+ 0x01,
		(UW)fatfs_read64	+ 0x01,
		(UW)fatfs_write64	+ 0x01,
		(UW)fatfs_truncate64	+ 0x01,
		(UW)fatfs_ftruncate64	+ 0x01,
		(UW)fatfs_fcntl64,
		(UW)fatfs_stat64_us,
		(UW)fatfs_fstat64_us,
		(UW)fatfs_utimes_us,
