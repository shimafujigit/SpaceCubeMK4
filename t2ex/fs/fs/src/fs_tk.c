...
LOCAL	INT	exe_svc_func(fs_env_t *env, INT fno, const void *par)
{
	...
	{	FS_FS_GETCWD_PARA *p = (FS_FS_GETCWD_PARA *)par;
		...
		if (p->size == 0 ||
			CheckSpaceRW(p->buf, (INT)p->size) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_GETDENTS_PARA *p = (FS_FS_GETDENTS_PARA *)par;
		...
		if (p->buf == NULL || p->bufsz <= 0 ||
			CheckSpaceRW(p->buf, (INT)p->bufsz) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_READ_PARA *p = (FS_FS_READ_PARA *)par;
		...
		if (p->count != 0 &&
			CheckSpaceRW(p->buf, (INT)p->count) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_WRITE_PARA *p = (FS_FS_WRITE_PARA *)par;
		...
		if (p->count != 0 &&
			CheckSpaceR((void *)p->buf, (INT)p->count) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_STAT_PARA *p = (FS_FS_STAT_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			CheckSpaceRW(p->buf, sizeof(*p->buf)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_STAT_US_PARA *p = (FS_FS_STAT_US_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			CheckSpaceRW(p->ubuf, sizeof(*p->ubuf)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_STAT_MS_PARA *p = (FS_FS_STAT_MS_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			CheckSpaceRW(p->mbuf, sizeof(*p->mbuf)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_STAT64_PARA *p = (FS_FS_STAT64_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			CheckSpaceRW(p->buf64, sizeof(*p->buf64)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_STAT64_US_PARA *p = (FS_FS_STAT64_US_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			CheckSpaceRW(p->ubuf64, sizeof(*p->ubuf64)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_STAT64_MS_PARA *p = (FS_FS_STAT64_MS_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			CheckSpaceRW(p->mbuf64, sizeof(*p->mbuf64)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_FSTAT_PARA *p = (FS_FS_FSTAT_PARA *)par;
		...
		if (CheckSpaceRW(p->buf, sizeof(*p->buf)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_FSTAT_US_PARA *p = (FS_FS_FSTAT_US_PARA *)par;
		...
		if (CheckSpaceRW(p->ubuf, sizeof(*p->ubuf)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_FSTAT_MS_PARA *p = (FS_FS_FSTAT_MS_PARA *)par;
		...
		if (CheckSpaceRW(p->mbuf, sizeof(*p->mbuf)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_FSTAT64_PARA *p = (FS_FS_FSTAT64_PARA *)par;
		...
		if (CheckSpaceRW(p->buf64, sizeof(*p->buf64)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_FSTAT64_US_PARA *p = (FS_FS_FSTAT64_US_PARA *)par;
		...
		if (CheckSpaceRW(p->ubuf64, sizeof(*p->ubuf64)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_FSTAT64_MS_PARA *p = (FS_FS_FSTAT64_MS_PARA *)par;
		...
		if (CheckSpaceRW(p->mbuf64, sizeof(*p->mbuf64)) < 0) {
			err = EX_INVAL;
		} else {
	...
	{	FS_FS_UTIMES_US_PARA *p = (FS_FS_UTIMES_US_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			(p->tim_u != NULL && ( CheckSpaceRW((void *)p->tim_u,
				sizeof(SYSTIM_U) * 2) < 0 ||
	...
	{	FS_FS_UTIMES_MS_PARA *p = (FS_FS_UTIMES_MS_PARA *)par;
		...
		if (ChkSpaceBstrR((void *)p->path, 0) < 0 ||
			(p->tim_m != NULL && CheckSpaceRW((void *)p->tim_m,
				sizeof(SYSTIM) * 2) < 0) ) {
			err = EX_INVAL;
			break;
		}
	...
	{	FS__FS_IOCTL_PARA *p = (FS__FS_IOCTL_PARA *)par;
		...
		if (p->arg != NULL) {
			...
			if (len > 0 && (
				((p->request & IOC_OUT) != 0 &&
					CheckSpaceRW(p->arg, len) < 0) ||
				((p->request & IOC_IN) != 0 &&
					CheckSpaceR(p->arg, len) < 0)) ) {
				err = EX_INVAL;
				break;
			}
		}
...
LOCAL	INT	fs_gdt_initialize(INT maxfile)
{
	...
	/* Create mlock */
	sts = CreateMLock(&fs_gdt.mlock, "Fsys");
exit0:
	return sts;
}
